...
Vec2 vec2_normalize(Vec2 v) {
    gdstk::Vec2 gv = {v.x, v.y};
    gv.normalize();  // This modifies gv in-place and returns the length
    return {gv.x, gv.y};
}
...
// Rotation functions (implement using basic math since C++ Vec2 doesn't have rotate)
Vec2 vec2_rotate(Vec2 v, double angle) {
    double cos_a = cos(angle);
    double sin_a = sin(angle);
    Vec2 result = {v.x * cos_a - v.y * sin_a, v.x * sin_a + v.y * cos_a};
    return result;
}
